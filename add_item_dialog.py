...
import sqlite3
from PyQt5.QtWidgets import QDialog, QLabel, QLineEdit, QPushButton, QVBoxLayout, QComboBox, QFormLayout, QMessageBox

class AddItemDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Add New Item to Stock')
    ...
    def add_item_to_database(self):
        ...
        if model_name and fournitures and stock_initial:
            try:
                ...
                c = conn.cursor()
                c.execute("INSERT INTO items (model_name, fournitures, ref_coul, stock_initial) VALUES (?, ?, ?, ?)",
                          (model_name, fournitures, ref, stock_initial))
                conn.commit()
                conn.close()
            ...
            QMessageBox.warning(self, "Missing Information", "Please fill in all the required fields.")
